...
try {
    ...
    $_SESSION['flash'] = ['type' => 'danger', 'message' => 'Failed to load medicine.'];
    redirect('/medicines/index.php');
}
$name = $medicine['name'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            $stmt = $pdo->prepare('UPDATE medicines SET name = :name, category_id = :category_id, quantity = :quantity, reorder_level = :reorder_level, expiry_date = :expiry_date WHERE id = :id');
            ...
            $stmt->execute();
            $_SESSION['flash'] = ['type' => 'success', 'message' => 'Medicine updated.'];
            redirect('/medicines/index.php');
        } catch (PDOException $e) {
            error_log($e->getMessage());
            $errors[] = 'Failed to update medicine. Please try again later.';
        }
    }
}
